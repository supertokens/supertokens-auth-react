                ...
                functions: (oI) => ({
                    ...
                    createNewSession: async function (input) {
                        return oI.createNewSession({
                            ...input,
                            claimsToLoad: input.claimsToLoad ?? [RolesClaim, EmailVerifiedClaim],
                        });
                    },
                }),
                apis: (oI) => ({
                    ...oI,
                    verifySession: async function (input) {
                        return oI.verifySession({
                            ...input,
                            verifySessionOptions: {
                                ...input.verifySessionOptions,
                                requiredClaims: input.verifySessionOptions.requiredClaims ?? [
                                    EmailVerifiedClaim.isVerified,
                                ],
                            },
                        });
                    },
...
app.post(
    ...
    verifySession({ requiredClaims: [] }), // This is the only endpoint that doesn't require email verification
    async (req, res) => {
        const session: SessionContainer = (req as any).session;
