  ...
  describe("Email Password test", function () {
    it("Successful signup with credentials", async function () {
      ...
      const userId = await page.evaluate(() =>
        window.__supertokensSessionRecipe.getUserId()
      );
      ...
      await page.goto(
        `${websiteDomain}/auth/verify-email?token=${tokenInfo.token}`
      );
      ...
      const sessionInfo = JSON.parse(
        alertText.replace(/^Session Information:/, "")
      );
