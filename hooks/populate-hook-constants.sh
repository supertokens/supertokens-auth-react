...
# auth-react keeps package.json and package-lock.json versions in lockstep; a
# mismatch means a botched bump, so refuse to tag/release. (Previously enforced
# inline in the "Check tag and branch correctness" step of the dev-tag
# pipeline.)
...
# For auth-react the released version IS the package version.
