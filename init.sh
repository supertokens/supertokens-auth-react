...
npm i -d --force && (cd ./test/server && npm i ) && (cd ./examples/for-tests && npm i -d --force && npm run link) && (cd ./examples/for-tests-react-16 && npm i -d --force --legacy-peer-deps && npm run link) && (cd ./test/with-typescript && npm i -d)
