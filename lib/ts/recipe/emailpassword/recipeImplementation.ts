...
export default class RecipeImplementation implements RecipeInterface {
    ...
    submitNewPassword = async (formFields: {
        id: string;
        value: string;
    }[], token: string, preAPIHook?: PreAPIHookFunction): Promise<SubmitNewPasswordAPIResponse> => {
        ...
        // then we call API
        const response: SubmitNewPasswordAPIResponse = await this.querier.post("/user/password/reset",
    ...
    sendPasswordResetEmail = async (formFields: {
        id: string;
        value: string;
    }[], preAPIHook?: PreAPIHookFunction): Promise<SendPasswordResetEmailAPIResponse> => {
        // first we validate on the frontend
        const validationErrors = await validateForm(
            formFields,
        ...
        // then we call API
        const response: SendPasswordResetEmailAPIResponse = await this.querier.post("/user/password/reset/token",
    ...
    signUp = async (formFields: {
        id: string;
        value: string;
    }[], preAPIHook?: PreAPIHookFunction): Promise<SignUpAPIResponse> => {
        // first we validate on the frontend
        const validationErrors = await validateForm(
            formFields,
        ...
        // then we call API
        const response: SignUpAPIResponse = await this.querier.post("/signup",
            { body: JSON.stringify({ formFields }) },
            preAPIHook
        );
    ...
    signIn = async (formFields: {
        id: string;
        value: string;
    }[], preAPIHook?: PreAPIHookFunction): Promise<SignInAPIResponse> => {
        // first we validate on the frontend
        const validationErrors = await validateForm(
            formFields,
        ...
        // then we call API
        const response: SignInAPIResponse = await this.querier.post("/signin",
            { body: JSON.stringify({ formFields }) },
            preAPIHook
        );
        return response;
    ...
    doesEmailExist = async (email: string, preAPIHook?: PreAPIHookFunction): Promise<boolean> => {
        const response: EmailExistsAPIResponse = await this.querier.get("/signup/email/exists",
            {}, { email }, preAPIHook);
