...
var SuperTokens = /*#__PURE__*/ (function() {
    ...
    function SuperTokens(config) {
        ...
        this.canHandleRoute = function() {
            var path = new _normalisedURLPath["default"](window.location.pathname);
            return _this.getPathsToComponentWithRecipeIdMap()[path.getAsStringDangerous()] !== undefined;
        };

        this.getRoutingComponent = function() {
            var path = new _normalisedURLPath["default"](window.location.pathname);
            var recipeId = (0, _utils.getRecipeIdFromSearch)(window.location.search);
            return _this.getMatchingComponentForRouteAndRecipeId(path.getAsStringDangerous(), recipeId);
        };

        this.getPathsToComponentWithRecipeIdMap = function() {
            if (_this.pathsToComponentWithRecipeIdMap) return _this.pathsToComponentWithRecipeIdMap;
        ...
        this.getMatchingComponentForRouteAndRecipeId = function(path, recipeId) {
            var routeComponents = SuperTokens.getPathsToComponentWithRecipeIdMap()[path];
            if (routeComponents === undefined) return undefined; // If recipeId provided, try to find a match.
