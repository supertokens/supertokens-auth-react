...
var getTranslationFunction = function () {
    ...
    var _store = utils.__spreadArray(utils.__spreadArray([defaultTranslationsCommon], stores, true), [userTranslationStore], false).reduce(utils.mergeObjects, {});
    ...
    var loadHandler = function (_eventName, detail) {
        setStore(utils.mergeObjects(_store, detail));
    };
...
var SuperTokens = /** @class */ (function () {
        ...
        for (var _i = 0, plugins_1 = plugins; _i < plugins_1.length; _i++) {
            ...
            if (plugin.overrideMap !== undefined) {
                for (var _j = 0, _k = Object.keys(plugin.overrideMap); _j < _k.length; _j++) {
                    var t = _k[_j];
                    this.componentOverrides[t] = utils.__assign(utils.__assign({}, this.componentOverrides[t]), ((_b = (_a = plugin.overrideMap[t]) === null || _a === void 0 ? void 0 : _a.components) !== null && _b !== void 0 ? _b : {}));
                }
            }
            this.componentOverrides.authRecipe = utils.__assign(utils.__assign({}, ((_c = this.componentOverrides.authRecipe) !== null && _c !== void 0 ? _c : {})), ((_d = plugin.generalAuthRecipeComponentOverrides) !== null && _d !== void 0 ? _d : {}));
        }
        this.pluginList = plugins.map(utils.getPublicPlugin);
        var publicConfig = utils.getPublicConfig(utils.__assign(utils.__assign({}, config), { appInfo: this.appInfo }));
        var _loop_1 = function (pluginIndex) {
            ...
            if (pluginInit && !this_1.pluginList[pluginIndex].initialized) {
                postSuperTokensInitCallbacks.PostSuperTokensInitCallbacks.addPostInitCallback(function () {
                    pluginInit(publicConfig, _this.pluginList, utils.package_version);
            ...
            if (pluginRouteHandlers) {
                ...
                if (typeof pluginRouteHandlers === "function") {
                    var result = pluginRouteHandlers(publicConfig, this_1.pluginList, utils.package_version);
    ...
    SuperTokens.init = function (config) {
        ...
        var normalisedAppInfo = utils.normaliseInputAppInfoOrThrowError(config.appInfo);
        var publicConfig = utils.getPublicConfig(utils.__assign(utils.__assign({}, config), { appInfo: normalisedAppInfo }));
        var finalPluginList = [];
        if ((_b = config.experimental) === null || _b === void 0 ? void 0 : _b.plugins) {
            for (var _i = 0, _c = config.experimental.plugins; _i < _c.length; _i++) {
                ...
                if (plugin.compatibleAuthReactSDKVersions) {
                    var versionCheck = isVersionCompatible(utils.package_version, plugin.compatibleAuthReactSDKVersions);
                    if (!versionCheck) {
                        throw new Error("Incompatible SDK version for plugin ".concat(plugin.id, ". Version \"").concat(utils.package_version, "\" not found in compatible versions: ").concat(JSON.stringify(plugin.compatibleAuthReactSDKVersions)));
                    }
                }
                if (plugin.dependencies) {
                    var result = plugin.dependencies(publicConfig, finalPluginList.map(utils.getPublicPlugin), utils.package_version);
                    if (result.status === "ERROR") {
                        throw new Error(result.message);
                    }
        ...
        for (var _d = 0, finalPluginList_1 = finalPluginList; _d < finalPluginList_1.length; _d++) {
            ...
            if (plugin.config) {
                var pluginConfig = plugin.config(utils.getPublicConfig(utils.__assign(utils.__assign({}, config), { appInfo: normalisedAppInfo }))) || {};
                ...
                config = utils.__assign(utils.__assign({}, config), pluginConfig);
            }
        }
        ...
        SuperTokens.instance = new SuperTokens(utils.__assign(utils.__assign({}, config), { recipeList: recipes }), finalPluginList);
