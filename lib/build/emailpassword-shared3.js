...
var getFunctionOverrides = function (onHandleEvent) {
    return function (originalImp) { return (utils.__assign(utils.__assign({}, originalImp), { submitNewPassword: function (input) {
            return utils.__awaiter(this, void 0, void 0, function () {
                var response;
                return utils.__generator(this, function (_a) {
                    switch (_a.label) {
                        case 0: return [4 /*yield*/, originalImp.submitNewPassword(utils.__assign(utils.__assign({}, input), { formFields: [input.formFields[0]] }))];
                        case 1:
                            response = _a.sent();
...
function normaliseEmailPasswordConfig(config) {
    ...
    return utils.__assign(utils.__assign({}, utils$1.normaliseAuthRecipe(config)), { signInAndUpFeature: signInAndUpFeature, resetPasswordUsingTokenFeature: resetPasswordUsingTokenFeature, override: override });
}
function normaliseSignInAndUpFeature(config) {
    ...
    var defaultSignInFields = signUpForm.formFields.reduce(function (signInFieldsAccumulator, field) {
        ...
        if (field.id === "password") {
            return utils.__spreadArray(utils.__spreadArray([], signInFieldsAccumulator, true), [
                utils.__assign(utils.__assign({}, field), { autoComplete: "current-password", validate: validators.defaultLoginPasswordValidator }),
            ], false);
        }
...
function normaliseSignInFormFeatureConfig(defaultFormFields, config) {
    ...
    if (config.formFields !== undefined) {
        userFormFields = config.formFields
            ...
            .map(function (field) { return (utils.__assign(utils.__assign({}, field), { optional: false })); });
    }
    var formFields = mergeFormFields(defaultFormFields, userFormFields);
    var style = config.style !== undefined ? config.style : "";
    return {
...
function normaliseResetPasswordUsingTokenFeature(signUpPasswordFieldValidate, signUpEmailField, config) {
    ...
    var enterEmailForm = {
        ...
        formFields: [
            utils.__assign(utils.__assign({}, getDefaultEmailFormField()), { validate: signUpEmailField.validate, placeholder: "", autofocus: true }),
        ],
    };
    return {
    ...
    for (var i = 0; i < userFormFields.length; i++) {
        ...
        for (var j = 0; j < mergedFormFields.length; j++) {
            ...
            if (userField.id === mergedField.id) {
                ...
                // Merge.
                mergedFormFields[j] = utils.__assign(utils.__assign(utils.__assign({}, mergedFormFields[j]), userField), { optional: optional });
                isNewField = false;
                break;
            }
        }
        ...
        if (isNewField) {
            mergedFormFields.push(utils.__assign({ optional: false, placeholder: userField.label, validate: validators.defaultValidate }, userField));
        }
    }
...
function getFormattedFormField(field) {
    ...
    return utils.__assign(utils.__assign({}, field), { validate: function (value) { return utils.__awaiter(_this, void 0, void 0, function () {
            return utils.__generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
        ...
        _this.getDefaultRedirectionURL = function (context) { return utils.__awaiter(_this, void 0, void 0, function () {
            return utils.__generator(this, function (_a) {
                if (context.action === "RESET_PASSWORD") {
                    return [2 /*return*/, utils.getDefaultRedirectionURLForPath(this.config, constants.DEFAULT_RESET_PASSWORD_PATH, context)];
                }
    ...
    EmailPassword.init = function (config) {
        ...
        return {
            ...
            authReact: function (appInfo) {
                EmailPassword.instance = new EmailPassword(utils.__assign(utils.__assign({}, normalisedConfig), { appInfo: appInfo, recipeId: EmailPassword.RECIPE_ID }));
                return EmailPassword.instance;
            },
            webJS: EmailPasswordWebJS__default.default.init(utils.__assign(utils.__assign({}, normalisedConfig), { override: {
