...
function getSuperTokensRoutesForReactRouterDom() {
    try {
        ...
        var withRouter_1 = require("react-router-dom").withRouter;
        var pathsToComponentWithRecipeIdMap = superTokens_1.default.getInstanceOrThrow().getPathsToComponentWithRecipeIdMap();
        return Object.keys(pathsToComponentWithRecipeIdMap).map(function (path) {
            return (React.createElement(Route_1, { exact: true, key: "st-" + path, path: path },
...
function SuperTokensRouteWithRecipeId(_a) {
    var path = _a.path, withRouter = _a.withRouter;
    ...
    var Component = superTokens_1.default.getInstanceOrThrow().getMatchingComponentForRouteAndRecipeId(normalisedPath, recipeId);
    if (Component === undefined) {
        return null;
    }
    var WithRouterComponent = withRouter(Component);
