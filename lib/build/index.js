...
var SuperTokensWrapper = function (props) {
    return jsxRuntime.jsx(
        session.SessionAuthWrapper,
        session.__assign({}, props, { requireAuth: false, doRedirection: false })
    );
};
...
var SuperTokensAPIWrapper = /** @class */ (function () {
    ...
    SuperTokensAPIWrapper.init = function (config) {
        session.SuperTokens.init(config);
    };
    SuperTokensAPIWrapper.canHandleRoute = function () {
        return session.SuperTokens.canHandleRoute();
    };
    SuperTokensAPIWrapper.getRoutingComponent = function () {
        return session.SuperTokens.getRoutingComponent();
    };
    SuperTokensAPIWrapper.changeLanguage = function (language) {
        return session.SuperTokens.getInstanceOrThrow().changeLanguage(language);
    };
    SuperTokensAPIWrapper.loadTranslation = function (store) {
        return session.SuperTokens.getInstanceOrThrow().loadTranslation(store);
    };
    SuperTokensAPIWrapper.getSuperTokensRoutesForReactRouterDom = function (reactRouterDom) {
        return session.SuperTokens.getSuperTokensRoutesForReactRouterDom(reactRouterDom);
    };
    var _a;
    ...
    SuperTokensAPIWrapper.redirectToAuth = function (options) {
        return session.__awaiter(void 0, void 0, void 0, function () {
            var _b;
            return session.__generator(_a, function (_c) {
                return [
                    2 /*return*/,
                    session.SuperTokens.getInstanceOrThrow().redirectToAuth(
                        session.__assign(session.__assign({}, options), {
                            redirectBack:
    ...
    SuperTokensAPIWrapper.useTranslation = session.useTranslation;
    SuperTokensAPIWrapper.useUserContext = session.useUserContext;
...
exports.useTranslation = session.useTranslation;
exports.useUserContext = session.useUserContext;
