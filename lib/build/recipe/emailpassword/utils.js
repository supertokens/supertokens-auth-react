...
function normaliseSignInAndUpFeature(appInfo, config) {
    ...
    var defaultSignInFields = signUpForm.formFields.reduce(function(signInFieldsAccumulator, field) {
        if (field.id === _constants.MANDATORY_FORM_FIELDS_ID.EMAIL) {
            ...
            var validate =
                field.validate !== _validators.defaultSignUpEmailValidator
                    ? field.validate
                    : _validators.defaultEmailValidator;
...
function getDefaultEmailFormField() {
    return {
        ...
        validate: _validators.defaultSignUpEmailValidator,
        optional: false,
        autoComplete: "email"
    };
}
...
function normaliseResetPasswordUsingTokenFeature(
    signUpPasswordFieldValidate,
    signUpEmailField,
    websiteBasePath,
    config
) {
    ...
    var validate =
        signUpEmailField.validate !== _validators.defaultSignUpEmailValidator
    ...
    var enterEmailForm = {
        ...
        formFields: [
            _objectSpread(
                _objectSpread({}, signUpEmailField),
                {},
                {
                    validate: validate
                }
