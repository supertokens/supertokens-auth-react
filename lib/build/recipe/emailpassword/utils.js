...
function normaliseEmailPasswordConfig(config) {
    ...
    var signUpPasswordField = signInAndUpFeature.signUpForm.formFields.find(function (field) {
        return field.id === constants_1.MANDATORY_FORM_FIELDS_ID.PASSWORD;
    });
    var signUpEmailField = signInAndUpFeature.signUpForm.formFields.find(function (field) {
        return field.id === constants_1.MANDATORY_FORM_FIELDS_ID.EMAIL;
    });
...
function normaliseSignInAndUpFeature(appInfo, config) {
    ...
    var defaultSignInFields = signUpForm.formFields.reduce(function (signInFieldsAccumulator, field) {
        if (field.id === constants_1.MANDATORY_FORM_FIELDS_ID.EMAIL) {
            return signInFieldsAccumulator.concat([field]);
        }
        if (field.id === constants_1.MANDATORY_FORM_FIELDS_ID.PASSWORD) {
            return signInFieldsAccumulator.concat([
                __assign({}, field, { autoComplete: "current-password", validate: validators_1.defaultLoginPasswordValidator })
            ]);
        }
...
function normaliseSignInFormFeatureConfig(appInfo, defaultFormFields, config) {
    ...
    if (config.formFields !== undefined) {
        userFormFields = config.formFields
            ...
            .filter(function (field) { return constants_1.MANDATORY_FORM_FIELDS_ID_ARRAY.includes(field.id); })
            // Sign In fields are never optional.
            .map(function (field) { return (__assign({}, field, { optional: false })); });
    }
    var formFields = mergeFormFields(defaultFormFields, userFormFields);
    var resetPasswordURL;
    if (config.resetPasswordURL !== undefined) {
        resetPasswordURL = new normalisedURLPath_1.default(config.resetPasswordURL);
    }
    else {
        resetPasswordURL = new normalisedURLPath_1.default("" + appInfo.websiteBasePath.getAsStringDangerous() + constants_1.DEFAULT_RESET_PASSWORD_PATH);
    }
    var style = config.style !== undefined ? config.style : {};
    return {
        style: style,
...
function normaliseEmailVerificationFeature(config) {
    ...
    var disableDefaultImplementation = config.disableDefaultImplementation === true;
    var mode = constants_1.EMAIL_VERIFICATION_MODE.OFF;
    if (config.mode === "REQUIRED") {
        mode = constants_1.EMAIL_VERIFICATION_MODE.REQUIRED;
    }
    ...
    for (var i = 0; i < userFormFields.length; i++) {
        ...
        for (var j = 0; j < mergedFormFields.length; j++) {
            ...
            if (userField.id === mergedField.id) {
                ...
                // Merge.
                mergedFormFields[j] = __assign({}, mergedFormFields[j], userField, { optional: optional });
                isNewField = false;
                break;
            }
        }
        ...
        if (isNewField) {
            mergedFormFields.push(__assign({ optional: false, placeholder: userField.label, validate: validators_1.defaultValidate }, userField));
        }
    }
...
function getFormattedFormField(field) {
    var _this = this;
    return __assign({}, field, { validate: function (value) { return __awaiter(_this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        // Absent or not optional empty field
                        if (value === "" && field.optional === false) {
                            return [2 /*return*/, "Field is not optional"];
                        }
