...
function normaliseThirdPartyConfig(config, allowEmptyProviders) {
    if (allowEmptyProviders === void 0) { allowEmptyProviders = false; }
    var signInAndUpFeature = normaliseSignInAndUpFeature(config.signInAndUpFeature, allowEmptyProviders);
    return __assign({}, utils_2.normaliseRecipeModuleConfig(config), { signInAndUpFeature: signInAndUpFeature });
}
...
function getOAuthState() {
    try {
        var state = JSON.parse(utils_1.getWindowOrThrow().sessionStorage.getItem(constants_1.SESSION_STORAGE_STATE_KEY));
        if (state === null) {
            return undefined;
        }
        return state;
