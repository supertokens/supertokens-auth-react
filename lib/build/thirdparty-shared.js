...
var getFunctionOverrides = function (recipeId, onHandleEvent) {
    return function (originalImp) { return (logger.__assign(logger.__assign({}, originalImp), { signInAndUp: function (input) {
            ...
            return originalImp.setStateAndOtherInfoToStorage({
                state: logger.__assign(logger.__assign({}, input.state), { rid: recipeId, oauth2LoginChallenge: loginChallenge, redirectToPath: genericComponentOverrideContext.getRedirectToPathFromURL() }),
                userContext: input.userContext,
            });
        } })); };
};
