...
function normaliseAuthRecipe(config) {
    return superTokens.normaliseRecipeModuleConfig(config);
}
function selectComponentsToCoverAllFirstFactors(comps, firstFactorIds) {
    ...
    var _loop_1 = function (i) {
        ...
        var factorIdsLeft = firstFactorIds.filter(function (id) { return !c.factorIds.includes(id); });
        ...
        var componentsLeft = comps.filter(function (c) { return c.factorIds.every(function (id) { return factorIdsLeft.includes(id); }); });
        ...
        if (componentsLeft.length !== 0) {
            ...
            if (nextComps !== undefined) {
                return { value: logger.__spreadArray([c], nextComps, true) };
            }
        }
    };
