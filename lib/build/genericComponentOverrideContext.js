...
function normaliseRecipeModuleConfig(config) {
    ...
    return exports.__assign(exports.__assign({}, config), { getRedirectionURL: getRedirectionURL, onHandleEvent: onHandleEvent, preAPIHook: preAPIHook, postAPIHook: postAPIHook, recipeRootStyle: rootStyle });
}

function normaliseMultitenancyConfig(config) {
    return exports.__assign(exports.__assign({}, normaliseRecipeModuleConfig(config)), { override: exports.__assign({ functions: function (originalImplementation) { return originalImplementation; } }, config === null || config === void 0 ? void 0 : config.override) });
}
    ...
    Multitenancy.init = function (config) {
        ...
        return {
            ...
            authReact: function (appInfo) {
                Multitenancy.instance = new Multitenancy(exports.__assign(exports.__assign({}, normalisedConfig), { appInfo: appInfo, recipeId: Multitenancy.RECIPE_ID }));
...
var getTranslationFunction = function () {
    var stores = [];
    for (var _i = 0; _i < arguments.length; _i++) {
        stores[_i] = arguments[_i];
    }
    var _a = SuperTokens.getInstanceOrThrow().languageTranslations, translationEventSource = _a.translationEventSource, userTranslationFunc = _a.userTranslationFunc, defaultLanguage = _a.defaultLanguage, userTranslationStore = _a.userTranslationStore;
    var _store = __spreadArray(__spreadArray([defaultTranslationsCommon], stores, true), [userTranslationStore], false).reduce(mergeObjects, {});
    var getStore = function () { return _store; };
    var setStore = function (newStore) {
        _store = newStore;
    };
    var _language = defaultLanguage;
    var getLanguage = function () { return _language; };
    ...
    void getCurrentLanguageFromCookie().then(function (cookieLanguage) {
        if (cookieLanguage) {
    ...
    var translate = function (key, replacements) {
        ...
        if (getLanguage() !== undefined) {
            var res = getStore()[getLanguage()] && getStore()[getLanguage()][key];
            var fallback = getStore()[defaultLanguage] && getStore()[defaultLanguage][key];
            if (res === undefined) {
            ...
            return res.replace(/{(\w+)}/g, function (match, p1) { return replacements[p1] || match; });
        }
...
var SuperTokens = /** @class */ (function () {
     ...
     * Constructor.
     */
    function SuperTokens(config, plugins) {
        var _this = this;
        ...
        this.changeLanguage = function (lang) { return __awaiter(_this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
        ...
        this.redirectToAuth = function (options) { return __awaiter(_this, void 0, void 0, function () {
        ...
        this.redirectToUrl = function (redirectUrl, navigate) { return __awaiter(_this, void 0, void 0, function () {
            return __generator(this, function (_a) {
                doRedirection(this.appInfo, redirectUrl, navigate);
                return [2 /*return*/];
            });
        }); };
        this.redirect = function (context, navigate, queryParams, userContext) { return __awaiter(_this, void 0, void 0, function () {
            var redirectUrl;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    ...
                    case 1:
                        redirectUrl = _a.sent();
                        if (redirectUrl === null) {
                            logDebugMessage("Skipping redirection because the user override returned null for context ".concat(JSON.stringify(context, null, 2)));
                            return [2 /*return*/];
                        }
                        redirectUrl = appendQueryParamsToURL(redirectUrl, queryParams);
        ...
        for (var _i = 0, plugins_1 = plugins; _i < plugins_1.length; _i++) {
            ...
            if (plugin.overrideMap !== undefined) {
                for (var _j = 0, _k = Object.keys(plugin.overrideMap); _j < _k.length; _j++) {
                    var t = _k[_j];
                    this.componentOverrides[t] = exports.__assign(exports.__assign({}, this.componentOverrides[t]), ((_b = (_a = plugin.overrideMap[t]) === null || _a === void 0 ? void 0 : _a.components) !== null && _b !== void 0 ? _b : {}));
                }
            }
            this.componentOverrides.authRecipe = exports.__assign(exports.__assign({}, ((_c = this.componentOverrides.authRecipe) !== null && _c !== void 0 ? _c : {})), ((_d = plugin.generalAuthRecipeComponentOverrides) !== null && _d !== void 0 ? _d : {}));
        }
        this.pluginList = plugins.map(getPublicPlugin);
        var publicConfig = getPublicConfig(exports.__assign(exports.__assign({}, config), { appInfo: this.appInfo }));
        var _loop_1 = function (pluginIndex) {
            var _l;
            var pluginInit = plugins[pluginIndex].init;
            if (pluginInit && !this_1.pluginList[pluginIndex].initialized) {
                postSuperTokensInitCallbacks.PostSuperTokensInitCallbacks.addPostInitCallback(function () {
                    pluginInit(publicConfig, _this.pluginList, package_version);
                    _this.pluginList[pluginIndex].initialized = true;
                });
            }
            var pluginRouteHandlers = plugins[pluginIndex].routeHandlers;
            if (pluginRouteHandlers) {
                var handlers = [];
                if (typeof pluginRouteHandlers === "function") {
                    var result = pluginRouteHandlers(publicConfig, this_1.pluginList, package_version);
                ...
                (_l = this_1.pluginRouteHandlers).push.apply(_l, handlers);
            }
        };
        var this_1 = this;
        ...
        this.rootStyle = (_e = config.style) !== null && _e !== void 0 ? _e : "";
        this.privacyPolicyLink = config.privacyPolicyLink;
        this.termsOfServiceLink = config.termsOfServiceLink;
        this.useShadowDom = (_f = config.useShadowDom) !== null && _f !== void 0 ? _f : true;
        this.defaultToSignUp = (_g = config.defaultToSignUp) !== null && _g !== void 0 ? _g : false;
        this.disableAuthRoute = (_h = config.disableAuthRoute) !== null && _h !== void 0 ? _h : false;
    }
    /*
     * Static Methods.
     */
    SuperTokens.init = function (config) {
        ...
        var normalisedAppInfo = normaliseInputAppInfoOrThrowError(config.appInfo);
        var publicConfig = getPublicConfig(exports.__assign(exports.__assign({}, config), { appInfo: normalisedAppInfo }));
        var finalPluginList = [];
        if ((_b = config.experimental) === null || _b === void 0 ? void 0 : _b.plugins) {
            for (var _i = 0, _c = config.experimental.plugins; _i < _c.length; _i++) {
                var plugin = _c[_i];
                if (plugin.compatibleAuthReactSDKVersions) {
                    var versionCheck = isVersionCompatible(package_version, plugin.compatibleAuthReactSDKVersions);
                    if (!versionCheck) {
                        throw new Error("Incompatible SDK version for plugin ".concat(plugin.id, ". Version \"").concat(package_version, "\" not found in compatible versions: ").concat(JSON.stringify(plugin.compatibleAuthReactSDKVersions)));
                    }
                }
                if (plugin.dependencies) {
                    var result = plugin.dependencies(publicConfig, finalPluginList.map(getPublicPlugin), package_version);
                    if (result.status === "ERROR") {
                        throw new Error(result.message);
                    }
        ...
        var duplicatePluginIds = finalPluginList.filter(function (plugin, index) {
            return finalPluginList.some(function (elem, idx) { return elem.id === plugin.id && idx !== index; });
        });
        if (duplicatePluginIds.length > 0) {
            throw new Error("Duplicate plugin IDs: ".concat(duplicatePluginIds.map(function (plugin) { return plugin.id; }).join(", ")));
        }
        for (var _d = 0, finalPluginList_1 = finalPluginList; _d < finalPluginList_1.length; _d++) {
            var plugin = finalPluginList_1[_d];
            if (plugin.config) {
                var pluginConfig = plugin.config(getPublicConfig(exports.__assign(exports.__assign({}, config), { appInfo: normalisedAppInfo }))) || {};
                ...
                config = exports.__assign(exports.__assign({}, config), pluginConfig);
            }
        }
        SuperTokensWebJS__default.default.init(exports.__assign(exports.__assign({}, config), { recipeList: recipes.map(function (_a) {
                var webJS = _a.webJS;
                return webJS;
            }), experimental: {
                plugins: finalPluginList.map(function (plugin) { return ({
                    id: plugin.id,
                    overrideMap: plugin.overrideMap,
                }); }),
            } }));
        SuperTokens.instance = new SuperTokens(exports.__assign(exports.__assign({}, config), { recipeList: recipes }), finalPluginList);
        postSuperTokensInitCallbacks.PostSuperTokensInitCallbacks.runPostInitCallbacks();
    };
    SuperTokens.getInstance = function () {
...
var createGenericComponentsOverrideContext = function (v, key) {
    ...
    var useComponentsOverrideContext = function () {
        ...
        return exports.__assign(exports.__assign({}, (_a = SuperTokens.getInstance()) === null || _a === void 0 ? void 0 : _a.componentOverrides[key]), contextValue);
    };
