 ...
 * under the License.
 */
var package_version = "0.48.0-canary-plugins.0";
...
function applyPlugins(recipeId, config, plugins) {
    var _a;
    var _config = exports.__assign({}, (config !== null && config !== void 0 ? config : {}));
    var functionLayers = [(_a = _config.override) === null || _a === void 0 ? void 0 : _a.functions];
    for (var _i = 0, plugins_1 = plugins; _i < plugins_1.length; _i++) {
        var plugin = plugins_1[_i];
        var overrides = plugin[recipeId];
        if (overrides) {
            _config = exports.__assign({}, (overrides.config ? overrides.config(_config) : _config));
            if (overrides.functions !== undefined) {
                functionLayers.push(overrides.functions);
            }
        }
    }
    functionLayers = functionLayers.reverse().filter(function (layer) { return layer !== undefined; });
    if (functionLayers.length > 0) {
        _config.override = exports.__assign(exports.__assign({}, _config.override), { functions: function (oI, builder) {
...
function getPublicConfig(config) {
    config.experimental; config.recipeList; var publicConfig = __rest(config, ["experimental", "recipeList"]);
    return publicConfig;
}
    ...
    Multitenancy.init = function (config) {
        return {
            ...
            authReact: function (appInfo, _, overrideMaps) {
                var normalisedConfig = normaliseMultitenancyConfig(applyPlugins(Multitenancy.RECIPE_ID, config, overrideMaps !== null && overrideMaps !== void 0 ? overrideMaps : []));
            ...
            webJS: function () {
                ...
                var init = MultitenancyWebJS__default.default.init(exports.__assign({}, normalisedConfig));
...
var SuperTokens = /** @class */ (function () {
    ...
    function SuperTokens(config, plugins) {
        ...
        var _loop_1 = function (pluginIndex) {
            ...
            if (pluginInit && !this_1.pluginList[pluginIndex].initialized) {
                postSuperTokensInitCallbacks.PostSuperTokensInitCallbacks.addPostInitCallback(function () {
                    pluginInit(config, _this.pluginList, package_version);
            ...
            if (pluginRouteHandlers) {
                ...
                if (typeof pluginRouteHandlers === "function") {
                    var result = pluginRouteHandlers(config, this_1.pluginList, package_version);
        ...
        var overrideMaps = plugins
            .filter(function (p) { return p.overrideMap !== undefined; })
            .map(function (p) { return p.overrideMap; });
        ...
        this.recipeList = config.recipeList.map(function (_a) {
            ...
            return authReact(_this.appInfo, enableDebugLogs, overrideMaps);
        });
    ...
    SuperTokens.init = function (config) {
        ...
        if ((_b = config.experimental) === null || _b === void 0 ? void 0 : _b.plugins) {
            for (var _i = 0, _c = config.experimental.plugins; _i < _c.length; _i++) {
                ...
                if (plugin.compatibleAuthReactSDKVersions) {
                    var versionContraints = Array.isArray(plugin.compatibleAuthReactSDKVersions)
                ...
                if (plugin.dependencies) {
                    var result = plugin.dependencies(getPublicConfig(config), finalPluginList.map(getPublicPlugin), package_version);
                    if (result.status === "ERROR") {
                        throw new Error(result.message);
                    }
        ...
        for (var _d = 0, finalPluginList_1 = finalPluginList; _d < finalPluginList_1.length; _d++) {
            ...
            if (plugin.config) {
                config = exports.__assign(exports.__assign({}, config), plugin.config(getPublicConfig(config)));
            }
        }
